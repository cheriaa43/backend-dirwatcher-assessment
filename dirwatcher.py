"""
Objectives
 A long-running program that demonstrates OS signal handling
(SIGTERM, SIGINT) and program logging. Use exception handling
to keep the program running. Structure your code repository
...
def watch_dir(args):
    """
    Look at the directory being watched and get a list of files
    Add files to files dictionary if they're not there already
    Log a message if you're adding something to dictionary that doesn't already exist--log as a new file
...
def find_magic(filename, starting_line, magic_word):
    ...
    # Key will be the filenames and the values will be the starting line you used and the last position you read
    """
    line_number = 0
...
def signal_handler(sig_num, frame):
    """
    Handler for SIGTERM and SIGINT. Other signals can be mapped here also (SIGHUP?)
